...
Everything you might want to change lives in ROWS and HEAD below — the
rest is layout. Deliberately no GitHub statistics here: the heatmap above
already covers those, so this panel is for the things a number cannot say.
...
HEAD = ("alessio", "builtwithsabba")

# (label, value) — an empty label continues the previous one.
ROWS = [
    ("Role",    "AI Builder & Founder"),
    ("Now",     "FiscEdge — AI-powered company creation"),
    ("",        "BuiltWithSabba — build log & experiments"),
    ("Prev",    "Delos / The Italians · NEXUS LINK LLC (US)"),
    ("Shipped", "FiscEdge Academy · Giurimi · The Italians"),
    ("Stack",   "TypeScript · Next.js · React · Tailwind"),
    ("Infra",   "Supabase · Vercel · GitHub Actions · Python"),
    ("AI",      "LLM integration · agentic pipelines"),
    ("Studies", "MSc Entrepreneurship · MBA · BSc Chemistry"),
    ("Langs",   "IT native · EN C1 · ES basic"),
    ("Loc",     "Rome, IT — building globally"),
    ("Web",     "builtwithsabba.com"),
]

MOTTO = "Build with AI. Think like a founder."

# neofetch prints its palette at the bottom; these are the site's own.
...
def line(y, label, value, delay):
    ...
    if label:
        out += (f'<text x="{PAD_X}" y="{y:.0f}" font-family="{k.MONO}" '
                f'font-size="{FS:g}" fill="{k.ACCENT}" '
...
def build():
    ...
    body.append(
        ...
        f'<tspan fill="{k.ACCENT}">{host}</tspan></text></g>'
        if not STATIC else
    ...
    cls = "" if STATIC else ' class="r"'
    body.append(
        f'    <text{cls}{d} x="{PAD_X}" y="{y:.0f}" font-family="{k.MONO}" '
        f'font-size="11.5" fill="{k.DIM}" '
    ...
    return k.panel(W, h, "sabba@github: ~/neofetch",
